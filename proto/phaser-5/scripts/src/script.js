...
function preload() {
    ...
    game.load.tilemap(      'new-level',    'assets/imgs/in-form-tall-cursor.json', null, Phaser.Tilemap.TILED_JSON );
    game.load.spritesheet(  'dude',         'assets/imgs/cursor-sprites.png', 32, 48 );
    game.load.image(        'tiles',        'assets/imgs/tiles.png', 32, 48 );
...
function create() {
    ...
    // smaller
    game.add.sprite( 80, 30, 'label' ).scale.set( .75 );
    game.add.sprite( 80, 330, 'label-2' ).scale.set( .75 );
    ...
    // standing size
    player.body.setSize( 4, 32, 14, 16 );
    ...
    player.body.setJumping = function() {
        player.body.setSize( 16, 24, 8, 13 );
    ...
    player.scale.y = 3;
    // add scale to gui
    gui.add( player.scale, 'y', .5, 3 ).step( .25 ).onFinishChange( restart );
    ...
    map.createFromObjects('Asterisks', 26, 'asterisk', 0, true, false, asterisks);
...
function update() {
    ...
    if( !player.isDead ) {
        ...
        if( player.body.onFloor() && player.body.state == 'jumping' )
        {
